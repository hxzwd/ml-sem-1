...
static void printUsageAndExit(const char* argv0)
{
	...
	<< "  -da   | --disassemble-code            Print dissambled BIOS code to stdout\n"
	<< std::endl;
...
int main(int argc, char** argv)
{
	...
	for (size_t i = 2; i < args.size(); ++i)
	{
		...
		if (args[i] == "-rt" || args[i] == "--run-testing")
			runTesting = true;
		if (args[i] == "-da" || args[i] == "--disassemble-code")
			flag_disasm = true;
	}
	...
	cpu.m_disasm.set_state(flag_disasm);
	cpu.m_disasm.set_format("full");
	cpu.m_disasm.set_format("single-line");
